...
const SPEC_URL =
  process.argv[2] || "https://app.heymantle.com/openapi/core-api.json";
...
const HELPERS_TS = path.resolve(ROOT, "src/types/helpers.ts");
...
function looksPlural(s: string): boolean {
  return (
    (s.endsWith("s") && !s.endsWith("ss") && !s.endsWith("us")) ||
    s.endsWith("ies")
  );
}
...
function startsWithActionVerb(s: string): boolean {
  ...
  return (
    SINGLE_WORD_ACTIONS.has(firstWord) ||
    [
      "add",
      "remove",
      "set",
...
function deriveMethodName(
  ...
  if (methodNaming === "last-segment") {
    ...
    if (namedSegs.length > 0)
      return snakeToCamel(namedSegs[namedSegs.length - 1].val);
  }
  ...
  if (lastSeg.kind === "param") {
    if (namedSegs.length === 0) {
      ...
      if (httpMethod === "put" && allMethodsOnPath.includes("patch"))
        return "replace";
      if (httpMethod === "patch" && allMethodsOnPath.includes("put"))
        return "update";
      return map[httpMethod] ?? httpMethod;
    }
    const subNoun = toPascalCase(singularize(namedSegs[namedSegs.length - 1]));
    if (httpMethod === "put" && allMethodsOnPath.includes("patch"))
      return `replace${subNoun}`;
    if (httpMethod === "patch" && allMethodsOnPath.includes("put"))
      return `update${subNoun}`;
    const map: Record<string, string> = {
      get: `get${subNoun}`,
      put: `update${subNoun}`,
      patch: `update${subNoun}`,
      delete: `delete${subNoun}`,
      post: `create${subNoun}`,
    };
  ...
  for (let i = lastIdx - 1; i >= 0; i--) {
    if (parsed[i].kind === "named") {
      contextNoun = parsed[i].val;
      break;
    }
  }
...
function resolveParam(
  ...
  paramOrRef: Record<string, unknown>,
): Record<string, unknown> {
  if (paramOrRef.$ref) {
    const refName = (paramOrRef.$ref as string).split("/").pop()!;
    return (
      (spec.components as Record<string, unknown>).parameters as Record<
        string,
        unknown
      >
...
function getQueryInfo(
  ...
  pathStr: string,
  method: string,
): { hasQuery: boolean; queryRequired: boolean } {
  const pathDef = (spec.paths as Record<string, Record<string, unknown>>)[
    pathStr
  ];
  const pathLevel = (pathDef.parameters as unknown[] | undefined) ?? [];
  const opLevel =
    ((pathDef[method] as Record<string, unknown>)?.parameters as
      | unknown[]
      | undefined) ?? [];
  const all = [...pathLevel, ...opLevel].map((p) =>
    resolveParam(spec, p as Record<string, unknown>),
  );
...
function getBodyInfo(
  ...
  pathStr: string,
  method: string,
): { hasBody: boolean; bodyRequired: boolean } {
  const op = (spec.paths as Record<string, Record<string, unknown>>)[pathStr][
    method
...
function generateMethod(
  ...
  // Build API call
  const HTTP = httpMethod.toUpperCase() as
    | "GET"
    | "POST"
    | "PUT"
    | "PATCH"
...
const RESOURCE_GROUPS: ResourceGroup[] = [
  {
    file: "affiliate-commissions",
    className: "AffiliateCommissionsResource",
    clientProp: "affiliateCommissions",
    singularName: "commission",
    pathPrefixes: ["/affiliate_commissions"],
  },
  {
    file: "affiliate-payouts",
    className: "AffiliatePayoutsResource",
    clientProp: "affiliatePayouts",
    singularName: "payout",
    pathPrefixes: ["/affiliate_payouts"],
  },
  {
    file: "affiliate-programs",
    className: "AffiliateProgramsResource",
    clientProp: "affiliatePrograms",
    singularName: "program",
    pathPrefixes: ["/affiliate_programs"],
  },
  {
    file: "affiliate-referrals",
    className: "AffiliateReferralsResource",
    clientProp: "affiliateReferrals",
    singularName: "referral",
    pathPrefixes: ["/affiliate_referrals"],
  },
  {
    file: "affiliates",
    className: "AffiliatesResource",
    clientProp: "affiliates",
    singularName: "affiliate",
    pathPrefixes: ["/affiliates"],
  },
  {
    file: "ai-agent-runs",
    className: "AiAgentRunsResource",
    clientProp: "aiAgentRuns",
    singularName: "run",
  ...
  {
    file: "assistant",
    className: "AssistantResource",
    clientProp: "assistant",
  ...
  {
    file: "companies",
    className: "CompaniesResource",
    clientProp: "companies",
    singularName: "company",
    pathPrefixes: ["/companies"],
  },
  {
    file: "contacts",
    className: "ContactsResource",
    clientProp: "contacts",
  ...
  {
    file: "custom-data",
    className: "CustomDataResource",
    clientProp: "customData",
  ...
  {
    file: "customer-segments",
    className: "CustomerSegmentsResource",
    clientProp: "customerSegments",
    singularName: "segment",
    pathPrefixes: ["/customer_segments"],
  },
  {
    file: "customers",
    className: "CustomersResource",
    clientProp: "customers",
    singularName: "customer",
    pathPrefixes: ["/customers"],
  },
  {
    file: "deal-activities",
    className: "DealActivitiesResource",
    clientProp: "dealActivities",
    singularName: "activity",
    pathPrefixes: ["/deal_activities"],
  },
  {
    file: "deal-flows",
    className: "DealFlowsResource",
    clientProp: "dealFlows",
    singularName: "dealFlow",
  ...
  {
    file: "email-campaigns",
    className: "EmailCampaignsResource",
    clientProp: "emailCampaigns",
  ...
  {
    file: "email-deliveries",
    className: "EmailDeliveriesResource",
    clientProp: "emailDeliveries",
    singularName: "delivery",
    pathPrefixes: ["/email/deliveries"],
  },
  {
    file: "email-layouts",
    className: "EmailLayoutsResource",
    clientProp: "emailLayouts",
    singularName: "layout",
    pathPrefixes: ["/email/layouts"],
  },
  {
    file: "email-senders",
    className: "EmailSendersResource",
    clientProp: "emailSenders",
    singularName: "sender",
    pathPrefixes: ["/email/senders"],
  },
  {
    file: "email-unsubscribe-groups",
    className: "EmailUnsubscribeGroupsResource",
    clientProp: "emailUnsubscribeGroups",
    singularName: "group",
    pathPrefixes: ["/email/unsubscribe_groups"],
  },
  {
    file: "entities",
    className: "EntitiesResource",
    clientProp: "entities",
    singularName: "entity",
  ...
  {
    file: "flow-extensions",
    className: "FlowExtensionsResource",
    clientProp: "flowExtensions",
  ...
  },
  {
    ...
    className: "JournalEntriesResource",
    clientProp: "journalEntries",
    singularName: "entry",
    pathPrefixes: ["/journal_entries"],
  },
  {
    file: "lists",
    className: "ListsResource",
    clientProp: "lists",
    singularName: "list",
    pathPrefixes: ["/lists"],
  },
  ...
  },
  {
    ...
    clientProp: "organization",
    singularName: "organization",
    pathPrefixes: ["/organization"],
  },
  {
    file: "subscriptions",
    className: "SubscriptionsResource",
    clientProp: "subscriptions",
    singularName: "subscription",
    pathPrefixes: ["/subscriptions"],
  },
  {
    file: "synced-emails",
    className: "SyncedEmailsResource",
    clientProp: "syncedEmails",
    singularName: "syncedEmail",
    pathPrefixes: ["/synced_emails"],
  },
  {
    file: "tasks",
    className: "TasksResource",
    clientProp: "tasks",
    singularName: "task",
    pathPrefixes: ["/tasks"],
  },
  {
    file: "tickets",
    className: "TicketsResource",
    clientProp: "tickets",
    singularName: "ticket",
    pathPrefixes: ["/tickets"],
  },
  {
    file: "timeline-comments",
    className: "TimelineCommentsResource",
    clientProp: "timelineComments",
    singularName: "comment",
    pathPrefixes: ["/timeline_comments"],
  },
  {
    file: "transactions",
    className: "TransactionsResource",
    clientProp: "transactions",
    singularName: "transaction",
    pathPrefixes: ["/transactions"],
  },
  {
    file: "usage-events",
    className: "UsageEventsResource",
    clientProp: "usageEvents",
    singularName: "event",
    pathPrefixes: ["/usage_events"],
  },
  {
    file: "users",
    className: "UsersResource",
    clientProp: "users",
    singularName: "user",
    pathPrefixes: ["/users"],
  },
...
function generateResourceFile(
  ...
  for (const { pathStr, method, prefix } of entries) {
    ...
    const raw = deriveMethodName(
      method,
      suffix,
      group.singularName,
      allMethodsOnPath,
      group.methodNaming ?? "standard",
    );
  ...
  for (const { pathStr, method, prefix } of entries) {
    ...
    if (count > 1) {
      ...
      finalName =
        group.methodOverrides?.[deduped] ??
        group.methodOverrides?.[name] ??
        deduped;
    } else {
  ...
  for (const { pathStr, method, prefix } of entries) {
    ...
    const suffix = pathStr.slice(prefix.length);
    const rawName = deriveMethodName(
      method,
      suffix,
      group.singularName,
      allMethodsOnPath,
  ...
  const needsPaths =
    methods.some((m) => m.includes("paths['")) ||
    group.customMethods?.includes("paths['");
  const needsComponents = group.customMethods?.includes("components[");
  const importsLine =
    needsPaths || needsComponents
...
// ─── helpers.ts generation ────────────────────────────────────────────────────

function generateHelpers(spec: Record<string, unknown>): string {
...
function buildClientImports(groups: ResourceGroup[]): string {
  return groups
    .map((g) => `import { ${g.className} } from './resources/${g.file}';`)
    .join("\n");
}

function buildClientProperties(groups: ResourceGroup[]): string {
  return groups
    .map((g) => `  public readonly ${g.clientProp}: ${g.className};`)
    .join("\n");
}

function buildClientInstantiations(groups: ResourceGroup[]): string {
  return groups
    .map((g) => `    this.${g.clientProp} = new ${g.className}(this);`)
    .join("\n");
}
...
function validateSpecCoverage(spec: Record<string, unknown>): void {
  ...
  if (uncovered.length === 0) {
    console.log(
      `  ✓ All ${specPaths.length} spec paths are covered by a resource group`,
    );
    return;
  }
  console.error(
...
async function main() {
  ...
  // ── 2b. Generate src/types/helpers.ts ────────────────────────────────────
  console.log("Generating types/helpers.ts…");
  fs.writeFileSync(HELPERS_TS, generateHelpers(spec));
  console.log(`  Written ${HELPERS_TS}`);
  ...
  if (fs.existsSync(oldCamelCase)) {
    ...
    console.log(
      "  Removed legacy timelineComments.ts (replaced by timeline-comments.ts)",
    );
  }
  ...
  for (const group of activeGroups) {
    ...
    if (!content) {
      console.warn(
        `  Warning: no spec paths found for ${group.file}, skipping`,
      );
      continue;
    }
  ...
  client = patchClientSection(
    client,
    "resource-imports",
    buildClientImports(RESOURCE_GROUPS),
  );
  client = patchClientSection(
    client,
    "resource-properties",
    buildClientProperties(RESOURCE_GROUPS),
  );
  client = patchClientSection(
    client,
    "resource-instantiations",
    buildClientInstantiations(RESOURCE_GROUPS),
  );
